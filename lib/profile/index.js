...
function defaults ( ) {
  var profile = {
    max_iob: 10 // if max_iob is not provided, will default to zero 
    ...
    , high_temptarget_raises_sensitivity: true // raise sensitivity for temptargets >= 111.  synonym for exercise_mode
    ...
    , resistance_lowers_target: true // lower BG target when autosens detects resistance
    , adv_target_adjustments: false // lower target automatically when BG and eventualBG are high
    , exercise_mode: true // when true, > 105 mg/dL high temp target adjusts sensitivityRatio for exercise_mode. This majorly changes the behavior of high temp targets from before. synonmym for high_temptarget_raises_sensitivity
    ...
    , unsuspend_if_no_temp: true // if true, pump will un-suspend after a zero temp finishes
...
function displayedDefaults () {
    ...
    profile.high_temptarget_raises_sensitivity = allDefaults.high_temptarget_raises_sensitivity;
    profile.low_temptarget_lowers_sensitivity = allDefaults.low_temptarget_lowers_sensitivity;
    profile.sensitivity_raises_target = allDefaults.sensitivity_raises_target;
    profile.resistance_lowers_target = allDefaults.resistance_lowers_target;
    profile.adv_target_adjustments = allDefaults.adv_target_adjustments;
    profile.exercise_mode = allDefaults.exercise_mode;
    profile.half_basal_exercise_target = allDefaults.half_basal_exercise_target;
    profile.maxCOB = allDefaults.maxCOB;
    profile.wide_bg_target_range = allDefaults.wide_bg_target_range;
    profile.skip_neutral_temps = allDefaults.skip_neutral_temps;
    profile.unsuspend_if_no_temp = allDefaults.unsuspend_if_no_temp;
    profile.bolussnooze_dia_divisor = allDefaults.bolussnooze_dia_divisor;
    profile.min_5m_carbimpact = allDefaults.min_5m_carbimpact;
    profile.autotune_isf_adjustmentFraction = allDefaults.autotune_isf_adjustmentFraction;
    profile.remainingCarbsFraction = allDefaults.remainingCarbsFraction;
    profile.remainingCarbsCap = allDefaults.remainingCarbsCap;
    profile.enableUAM = allDefaults.enableUAM;
    profile.A52_risk_enable = allDefaults.A52_risk_enable;
    ...
    profile.enableSMB_always = allDefaults.enableSMB_always;
    profile.enableSMB_after_carbs = allDefaults.enableSMB_after_carbs;
    profile.allowSMB_with_high_temptarget = allDefaults.allowSMB_with_high_temptarget;
    profile.maxSMBBasalMinutes = allDefaults.maxSMBBasalMinutes;
    profile.curve = allDefaults.curve;
    profile.useCustomPeakTime = allDefaults.useCustomPeakTime;
    profile.insulinPeakTime = allDefaults.insulinPeakTime;
    profile.offline_hotspot = allDefaults.offline_hotspot;
    profile.carbsReqThreshold = allDefaults.carbsReqThreshold;
    profile.offline_hotspot = allDefaults.offline_hotspot;
